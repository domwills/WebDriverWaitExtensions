Feature: Network RequestSent Condition
	...
	Scenario: RequestSent(url, action, out var condition) will out a Condition when the request is sent
	...
	Scenario: RequestSent(url, action, out var condition) will out a Condition if the request is not sent
