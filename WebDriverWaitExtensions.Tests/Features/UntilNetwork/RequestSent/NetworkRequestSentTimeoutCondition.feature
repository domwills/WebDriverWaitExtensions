Feature: Network RequestSent Timeout Condition
	...
	Scenario: RequestSent(url, action, timeout, out var condition) will return a Condition when the request is sent
	...
	Scenario: RequestSent(url, action, timeout, out var condition) will return a Condition if the request is not sent
